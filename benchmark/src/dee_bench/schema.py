...
RUNS = Table(
    ...
    columns=[
        ...
        Column("delivery", pa.string(), "'direct' if this run executed one DAG start to finish, 'resumed' if a candidate was cancelled at its budget and the run was finished under the search's incumbent. **Filter to 'direct' before comparing engine_wall_ms with anything**: a resumed run's second half started from a warm, half-built warehouse, so its wall time measures neither DAG."),
        Column("trial_elapsed_ms", pa.int64(), "On a resumed run, what the cancelled candidate had spent when it was stopped -- a lower bound on what it would have cost. Null on a direct delivery."),
        Column("resume_elapsed_ms", pa.int64(), "On a resumed run, what finishing under the incumbent took. Null on a direct delivery."),
...
PASS_ITERATIONS = Table(
    ...
    columns=[
        ...
        Column("runtime_ms", pa.int64(), "Measured runtime of this candidate. A lower bound only when outcome is 'cancelled' -- and then in the objective's own units: under hmp_objective=query_time the bound is a total-node-time cap, not wall clock, and the trial's real wall clock is in trial_ms. Do not compare this column across objectives without splitting on outcome."),
        ...
        Column("node_time_ms", pa.int64(), "Time the database spent executing this iteration's queries: every node's duration summed, across the trial and any resume that finished it. Exceeds runtime_ms wherever nodes ran concurrently."),
        Column("predicted_makespan_ms", pa.int64(), "Wall clock this candidate was predicted to have, recorded before it ran. Null when the cost model could not price the materialization -- a refusal, not a zero. Score it against runtime_ms where outcome is 'ok'."),
        Column("trial_ms", pa.int64(), "Wall time the candidate ran before being cancelled. Null unless outcome is 'cancelled'."),
        Column("resume_overhead_ms", pa.int64(), "Wall time between the cancellation and the incumbent picking the run back up: planning the resume and dropping what it cannot reuse. Null unless outcome is 'cancelled'."),
        Column("resume_ms", pa.int64(), "Wall time of the run that finished under the incumbent. Null unless outcome is 'cancelled'."),
        Column("total_ms", pa.int64(), "What this iteration actually cost end to end: trial_ms + resume_overhead_ms + resume_ms when cancelled, else runtime_ms. Unlike runtime_ms this is never a lower bound."),
