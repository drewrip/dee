...
pub async fn optimizer_options() -> Json<Vec<OptimizerOption>> {
    ...
    Json(vec![
        ...
        OptimizerOption { name: "run_nodefusion_pass", flag: "--enable nodefusion", kind: "bool",
            ...
            doc: "Share View work that several Tables repeat through one rollup node. A pure rewrite unless the adaptive search is on." },
        OptimizerOption { name: "nodefusion_materialize_ctes_override", flag: "--nodefusion-materialize-ctes-override", kind: "str_list",
            ...
            doc: "The exact set of node IDs whose CTEs NodeFusion materializes, replacing the rule of two or more readers inside the rollup." },
        OptimizerOption { name: "nodefusion_adaptive_materialize_ctes", flag: "--nodefusion-adaptive-materialize-ctes", kind: "bool",
            passes: &["nodefusion"], choices: None, default: json!(d.nodefusion_adaptive_materialize_ctes),
            doc: "Choose the materialized CTE set by measurement rather than by rule. Turns NodeFusion into a continuous search: it measures a baseline fusion, ranks candidate sets, and trials one per DAG run. Incompatible with nodefusion_materialize_ctes_override." },
        OptimizerOption { name: "nodefusion_objective", flag: "--nodefusion-objective", kind: "str",
            passes: &["nodefusion"], choices: Some(&["makespan", "query_time"]), default: json!(d.nodefusion_objective.as_str()),
            doc: "Which measure the adaptive search minimizes. Picks both the order candidate sets are trialled in and the test a trial has to pass." },
        OptimizerOption { name: "nodefusion_search_budget", flag: "--nodefusion-search-budget", kind: "int",
            passes: &["nodefusion"], choices: None, default: json!(d.nodefusion_search_budget),
            doc: "How many candidate CTE sets the adaptive search may price before spending DAG runs. EXPLAIN-only, and so far cheaper than measuring." },
        OptimizerOption { name: "nodefusion_max_runs", flag: "--nodefusion-max-runs", kind: "int",
            passes: &["nodefusion"], choices: None, default: json!(d.nodefusion_max_runs),
            doc: "DAG executions the adaptive search may spend on candidates, on top of the one it spends measuring the baseline." },
        OptimizerOption { name: "nodefusion_top_share", flag: "--nodefusion-top-share", kind: "float",
            passes: &["nodefusion"], choices: None, default: json!(d.nodefusion_top_share),
            doc: "The share of total ranking score the adaptive working set covers, in (0, 1]." },
        OptimizerOption { name: "nodefusion_cost_model", flag: "--nodefusion-cost-model", kind: "str",
            passes: &["nodefusion"], choices: Some(&["learned_cost", "cardinality", "operators"]), default: json!(d.nodefusion_cost_model.as_str()),
            doc: "How the adaptive search prices a region of a fused plan." },
        OptimizerOption { name: "nodefusion_spool_seconds_per_byte", flag: "--nodefusion-spool-seconds-per-byte", kind: "float",
            passes: &["nodefusion"], choices: None, default: json!(d.nodefusion_spool_seconds_per_byte),
            doc: "What spooling a MATERIALIZED CTE costs, in seconds per byte, replacing the modelled rate outright." },
        OptimizerOption { name: "nodefusion_spool_cost_factor", flag: "--nodefusion-spool-cost-factor", kind: "float",
            passes: &["nodefusion"], choices: None, default: json!(d.nodefusion_spool_cost_factor),
            doc: "What spooling a MATERIALIZED CTE costs, as a fraction of writing the same rows to a table. Defaults per backend (DuckDB 0.05, Postgres 0.35) -- a modelled guess, not a measurement." },
